...
(defn cmd-type
  ...
  (let [{:keys [session text pane enter delay]
    ...
    (when-not session
      ...
      (System/exit 1))
    (when-not text
      (binding [*out* *err*] (println "Error: text required"))
      (System/exit 1))
    (when-not (find-session session)
      (binding [*out* *err*] (println (str "Session '" session "' not found")))
      (System/exit 1))
...
(defn cmd-mouse
  ...
  (let [{:keys [session x y pane click double]
    ...
    (when-not session
      ...
      (System/exit 1))
    (when-not (and x y)
      (binding [*out* *err*] (println "Error: x and y coordinates required"))
      (System/exit 1))
    (when-not (find-session session)
      (binding [*out* *err*] (println (str "Session '" session "' not found")))
      (System/exit 1))
...
Commands:
  ...
  windows SESSION          List windows in a session
  panes SESSION            List panes in a session/window
  capture SESSION          Capture pane contents
  watch SESSION            Watch pane for changes
  send SESSION KEYS...     Send keys to a pane
  type SESSION TEXT        Type text into a pane
  mouse SESSION X Y        Send mouse click to pane
